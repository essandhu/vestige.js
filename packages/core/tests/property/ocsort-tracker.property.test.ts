...
const highScoreDetection: fc.Arbitrary<Detection> = positiveBBox.map((bbox) => ({
  bbox,
  score: 0.9,
}));

describe('OcSortTracker invariants', () => {
  it('all returned ids within a frame are unique', () => {
    fc.assert(
      fc.property(fc.array(highScoreDetection, { minLength: 0, maxLength: 8 }), (dets) => {
  ...
  it('frameIndex equals the number of update() calls', () => {
    fc.assert(
      fc.property(
        fc.array(fc.array(highScoreDetection, { maxLength: 4 }), { maxLength: 12 }),
        (frames) => {
  ...
  it('timeSinceUpdate equals exactly N after N consecutive missed frames', () => {
    ...
    // frame must advance tsu by exactly 1. A doubled-applyMiss bug would
    // make this fire at N=1 (observed tsu=2). maxAge=100 keeps the track
    // lost across the full N range so we never sample after removal.
    fc.assert(
      fc.property(fc.integer({ min: 1, max: 20 }), positiveBBox, (n, bbox) => {
        const t = new OcSortTracker({ minHits: 1, maxAge: 100 });
        t.update([{ bbox, score: 0.9 }]); // frame 1: spawn (tentative)
        t.update([{ bbox, score: 0.9 }]); // frame 2: match → confirmed (hitStreak=1 ≥ minHits=1)
  ...
  it('nextId never decreases across frames', () => {
    fc.assert(
      fc.property(
        fc.array(fc.array(highScoreDetection, { maxLength: 4 }), { maxLength: 10 }),
        (frames) => {
          const t = new OcSortTracker({ minHits: 1 });
          let maxIdSeen = 0;
